...
public sealed class ServiceAccountSeeder
{
    ...
    {
        var seeded = 0;
        seeded += await EnsureAsync(db, _admin.UserId, Role.Admin, _admin.Password, ct);
        seeded += await EnsureAsync(db, _realtor.UserId, Role.Realtor, _realtor.Password, ct);
        seeded += await EnsureAsync(db, _superAdmin.UserId, Role.SuperAdmin, _superAdmin.Password, ct);
    ...
    private async Task<int> EnsureAsync(
        AtriaDbContext db, Guid id, Role role, string password, CancellationToken ct)
    {

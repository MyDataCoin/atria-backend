...
/// <summary>
/// Publishes a property, opening it to investors. Restricted to Admins and applied immediately —
/// an administrator opening an offering is an ordinary editorial act, not something that waits on a
/// second pair of eyes.
/// </summary>
    ...
    : IRequestHandler<PublishPropertyCommand, Result>
{
    ...
    public async Task<Result> Handle(PublishPropertyCommand request, CancellationToken ct)
    {
        ...
        await _audit.WriteAsync(
            ...
            $"Объект «{property.Name}» опубликован", AuditSeverity.Success, ct);
